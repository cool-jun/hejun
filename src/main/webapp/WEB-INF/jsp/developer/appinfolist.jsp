...
<div class="row">
	<div class="col-md-12">
		<div class="x_panel">
			...
			<div class="x_content">
				<form method="post" action="showAllAppInfo">
			    ...
			    <ul>
					...
					<li>
						<div class="form-group">
							...
							<div class="col-md-6 col-sm-6 col-xs-12">
								<select name="queryPlatformId" class="form-control">
									<c:if test="${platFormList != null }">
									   <option value="">--请选择--</option>
									   <c:forEach var="dataDictionary" items="${platFormList}">
									   		<option <c:if test="${dataDictionary.valueId == queryPlatformId }">selected="selected"</c:if>
									   		value="${dataDictionary.valueId}">${dataDictionary.valueName}</option>
									   </c:forEach>
									</c:if>
        						</select>
							...
							<tbody>
								<c:forEach items="${appInfoList}" var="appInfo" varStatus="status">
									<tr role="row" class="odd">
										...
										<td><span id="appInfoStatus${appInfo.id}">${appInfo.statusName }</span>
										</td>
                        ...
                        <li>
                        	<c:choose>
								<c:when test="${appInfo.status == 2 || appInfo.status == 5}">
									<a class="saleSwitchOpen" saleSwitch="open" appinfoid=${appInfo.id }
											appsoftwarename=${appInfo.softwareName } data-toggle="tooltip"
									   data-placement="top" title="" data-original-title="恭喜您，您的审核已经通过，您可以点击上架发布您的APP">上架</a>
								</c:when>
								<c:when test="${appInfo.status == 4}">
									<a class="saleSwitchClose" saleSwitch="close" appinfoid=${appInfo.id }
											appsoftwarename=${appInfo.softwareName } data-toggle="tooltip"
									   data-placement="top" title="" data-original-title="您可以点击下架来停止发布您的APP，市场将不提供APP的下载">下架</a>
								</c:when>
							</c:choose>
                        </li>
                        <li><a class="addVersion" appinfoid="${appInfo.id }" data-toggle="tooltip" data-placement="top"
							   title="" data-original-title="新增APP版本信息">新增版本</a>
                        </li>
